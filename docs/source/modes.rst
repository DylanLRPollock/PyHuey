...
In **PyHuey**, this mode mirrors ``ChatGPT``, allowing you to chat with models like ``GPT-5``, ``GPT-4``, ``o1``, ``o3``, ``Claude``, ``Gemini``, ``Grok``, ``Perplexity (Sonar)``, ``Deepseek``, and more. It works with the OpenAI SDK using the ``Responses API`` and ``ChatCompletions API``. You can also use SDKs from Google GenAI, Anthropic, or xAI if the native SDK is enabled. You can set the endpoint for ``ChatCompletions`` in ``Config -> Settings -> API Keys``.
...
**PyHuey** enables quick and easy image creation with models like ``DALL-E 3``, ``gpt-image-1``, ``Imagen 3/4``, and ``Nano Banana``, as well as video generation using ``Veo3`` and ``Sora2``.
...
Images are stored in ``img`` directory in PyHuey's user data folder.
...
This mode expands on the basic chat functionality by including additional external tools like a ``Code Interpreter`` for executing code, ``Retrieval Files`` for accessing files, and custom ``Functions`` for enhanced interaction and integration with other APIs or services. In this mode, you can easily upload and download files. **PyHuey** streamlines file management, enabling you to quickly upload documents and manage files created by the model.

Setting up new assistants is simple - a single click is all it takes, and they instantly sync with the ``OpenAI API``. Importing assistants you've previously created with OpenAI into **PyHuey** is also a seamless process.
